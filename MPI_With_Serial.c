...
void recv_request(int rank)
{
		...
		while(i< size-2)
		{
			...
			if(flag)
			{
				...
				#endif
				MPI_Recv(recv_buf,512, MPI_CHAR, MPI_ANY_SOURCE, REQUEST, MPI_COMM_WORLD, &status);
			}
			else
			{
				continue;
...
void receive_response(int rank)
{
	...
	while(1)
	{
		...
		if(flag)
		{
			MPI_Recv(recv_buf,512, MPI_CHAR, MPI_ANY_SOURCE, RESPONSE, MPI_COMM_WORLD, &status);
		}
		else
		{
			continue;
